...
export function nurtureEmails(cfg, unsubscribeUrl) {
  return [
    ...
    { key:'day5', delayDays:5, subject:'Why the advanced version exists', html:shell('When the full system starts to help', `The advanced version is intended for businesses that need ${esc(cfg.advanced1)} or ${esc(cfg.advanced2)}.${button('Check current paid catalog', cfg.etsyUrl)}`, unsub(unsubscribeUrl)), text:`The full version is intended for ${cfg.advanced1} or ${cfg.advanced2}.\nCurrent paid catalog: ${cfg.etsyUrl}\n\nUnsubscribe: ${unsubscribeUrl}` },
    { key:'day8', delayDays:8, subject:'If you need a complete system, check what is live', html:shell('Need a more complete workflow?', `If the free starter is still enough, keep using it. If you now need the complete ${esc(cfg.paidName)} workflow, check the current catalog to see whether a matching validated product is live.${button('Check current paid catalog', cfg.etsyUrl)}`, unsub(unsubscribeUrl)), text:`If the free starter is still enough, keep using it. If you need the complete ${cfg.paidName}, check the current catalog: ${cfg.etsyUrl}\n\nUnsubscribe: ${unsubscribeUrl}` }
  ];
}
