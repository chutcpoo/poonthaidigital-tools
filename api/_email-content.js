...
export function deliveryEmail(cfg, downloadUrl) {
  ...
  const html = shell(title, `Your free starter is ready. It is designed to be useful on its own for ${esc(cfg.freeScope)}.${button('Download your free starter', downloadUrl)}<p style="margin-top:18px">If that solves the job, keep using the free version. The full Etsy version is only for workflows that need more structure or scale.</p>`);
...
export function nurtureEmails(cfg, unsubscribeUrl) {
  return [
    ...
    { key:'day5', delayDays:5, subject:'Why the advanced version exists', html:shell('When the full system starts to help', `The advanced version is intended for businesses that need ${esc(cfg.advanced1)} or ${esc(cfg.advanced2)}.${button('View the full version on Etsy', cfg.etsyUrl)}`, unsub(unsubscribeUrl)), text:`The full version is intended for ${cfg.advanced1} or ${cfg.advanced2}.\nEtsy: ${cfg.etsyUrl}\n\nUnsubscribe: ${unsubscribeUrl}` },
    { key:'day8', delayDays:8, subject:'If you need the complete system, here it is', html:shell('Ready for the complete workflow?', `If the free starter is still enough, keep using it. If you now need the complete ${esc(cfg.paidName)} workflow, you can see it on Etsy.${button('View on Etsy', cfg.etsyUrl)}`, unsub(unsubscribeUrl)), text:`If the free starter is still enough, keep using it. If you need the complete ${cfg.paidName}, see it here: ${cfg.etsyUrl}\n\nUnsubscribe: ${unsubscribeUrl}` }
  ];
}
