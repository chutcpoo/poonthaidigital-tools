(() => {
  ...
  const meta=(name,params={})=>{try{if(typeof window.pdTrackMeta==='function')window.pdTrackMeta(name,{lead_magnet:cfg.slug,...params});}catch(_){}};
  ...
  el.querySelector('#pdLeadForm').addEventListener('submit',async e=>{
    ...
    try{
      ...
      meta('Lead',{content_name:cfg.slug});
      if(consent){ga('marketing_opt_in');meta('MarketingOptIn',{content_name:cfg.slug});}
    }catch(_){
